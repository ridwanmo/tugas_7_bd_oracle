    ...
    <div class="main-panel">
      ...
      <?php
        ...
        if (isset($_GET['id'])) {
          ...
          $stid = oci_parse($con,"SELECT * FROM penjual_warung_kopi WHERE ID_PENJUAL = '$id'");
          oci_execute($stid);
         while (($d = oci_fetch_array($stid, OCI_BOTH)) != false) {
          ?>
      <form class="form-horizontal" action="proses_update_penjual.php" method="post">
       <div class="form-row">
        
        
          <input type="hidden" class="form-control" name="id_penjual" value="<?= $d['ID_PENJUAL'] ?>" >
        <div class="form-group col-md-6">
          <label for="">KODE PENJUAL</label>
          <input type="text" class="form-control" name="kode_penjual" value="<?= $d['KODE_PENJUAL'] ?>">
        </div>
        <div class="form-group col-md-6">
